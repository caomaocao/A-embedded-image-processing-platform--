...
namespace CaVeGen
{
    ...
    public partial class CassViewGenerator : Form, IMessageFilter
    {
        ...
        private void toolStripMenuItem_TreeView_Click(object sender, EventArgs e)
        {
            ...
            {
                this.dockManager.ShowContent(this.tabSolution_TreeView);
            }
            else
            {
        ...
        private void Comply()
        {
            try
            {
                ...
                {//编译无错
                    CreateXmlFile();
                    this.errorForm.errorList.Add(new string[] { null, "组态文件生成成功!", "info", null });
                }
                else
                {//编译出错
                    this.errorForm.errorList.Add(new string[] { null, "组态文件生成失败!", "info", null });
                }
        ...
        private void StartComply(bool DelXML)
        {
            ...
            if (DelXML)
            {//正常编译前删除XML
                DirectoryInfo SourceDirectory = new DirectoryInfo(Path.Combine(programPath, ProjectName));
                FileInfo[] Finfos = SourceDirectory.GetFiles();
        ...
        private bool CheckError(bool checkAddress)
        {
            ...
            //编译查错
            codeEditor.CodeEditor.checkIolist();
